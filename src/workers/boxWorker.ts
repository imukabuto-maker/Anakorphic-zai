...
function buildPanels(rawContours: Path[], config: BoxConfig): PanelData[] {
  const deviceClearance = config.deviceClearance ?? 0;
  ...
  rawContours.forEach(rawPath => {
    ...
    pData.forEach(p => {
      ...
      projected.forEach(proj => {
        ...
        // Device clearance: keep panel solid (no cuts) for the first deviceClearance mm from the wall side (y=0)
        const clipped  = clipPathToRect(localPath, clipXMin, clipXMax, deviceClearance, p.heightMm);
  ...
  // ── Invert Cutout: trim all panels to the deepest cut + 1 mm ─────────────
  if (config.invertCutout) {
    let maxCutY = deviceClearance; // fallback: at least the clearance zone
    for (const p of pData) {
      for (const seg of p.rawCutPaths) {
        for (const pt of seg) {
          if (pt.y > maxCutY) maxCutY = pt.y;
        }
      }
    }
    const newDepth = Math.min(config.depth, maxCutY + 1);

    for (const p of pData) {
      p.heightMm    = newDepth;
      p.outlinePath = buildPanelBoundary(p.widthMm, newDepth, p.panel, config);

      // Re-clip cut paths to the new (shorter) depth
      const trimmed: { cutPaths: string[]; rawCutPaths: Path[]; backProjectedPaths: Path[] } = {
        cutPaths: [], rawCutPaths: [], backProjectedPaths: [],
      };
      const offset = (p.panel === 'top' || p.panel === 'bottom') ? config.width / 2 : config.height / 2;
      p.rawCutPaths.forEach(seg => {
        const clipped = clipPathToRect(seg, 0, p.widthMm, deviceClearance, newDepth);
        clipped.forEach(c => {
          trimmed.cutPaths.push(pathsToSvgData([c], config.smoothEpsilon ?? 1.5));
          trimmed.rawCutPaths.push(c);
          trimmed.backProjectedPaths.push(
            backProjectToWall(c.map(pt => ({ x: pt.x - offset, y: pt.y })), p.panel, config),
          );
        });
      });
      p.cutPaths           = trimmed.cutPaths;
      p.rawCutPaths        = trimmed.rawCutPaths;
      p.backProjectedPaths = trimmed.backProjectedPaths;
    }
  }
